...
if (!quest) {
  console.log("Não tens nenhuma missão não concluída!");
} else {
  ...
  if (taskName === "WATCH_VIDEO" || taskName === "WATCH_VIDEO_ON_MOBILE") {
    ...
    if (!isApp) {
      ...
      api
        ...
        .then((res) => {
          ...
          console.log(
            `Seu jogo foi falsificado para ${applicationName}. Aguarde mais ${Math.ceil(
    ...
    if (!isApp) {
      ...
      console.log(
        `A tua transmissão foi falsificada para ${applicationName}. Transmite qualquer janela por mais ${Math.ceil(
          (secondsNeeded - secondsDone) / 60
        )} minutos.`
      );
      console.log(
        "Lembra-te de que precisas de pelo menos mais uma pessoa no canal de voz!"
      );
    }
